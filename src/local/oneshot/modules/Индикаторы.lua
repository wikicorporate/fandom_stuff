...
local Nameplate = {
    ...
    ["Спойлеры"] = {"<i class='fa-solid fa-triangle-exclamation'></i>Спойлеры", "mw-indicator__nameplate--spoiler", "Данная статья содержит спойлеры. Если не хотите испортить впечатление от игры, то настоятельно рекомендуем ''покинуть страницу''. [[Категория:Спойлеры]]"},
}

local Icon = {
    ["D"] = {"[[File:Bot.png]]", "class1", "Тултип"}
}
